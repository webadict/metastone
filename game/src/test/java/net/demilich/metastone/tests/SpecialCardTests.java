...
public class SpecialCardTests extends TestBase {
	...
	public void testGurubashiBerserker() {
		...
		Assert.assertEquals(attacker.getHp(), attacker.getMaxHp(context) - BASE_ATTACK);
		Assert.assertEquals(defender.getHp(), defender.getMaxHp(context) - attacker.getAttack(context));
		Assert.assertEquals(defender.getAttack(context), BASE_ATTACK + ATTACK_BONUS);

		// second attack, Gurubashi Berserker should become even stronger
		context.getLogic().performGameAction(mage.getId(), attackAction);
		Assert.assertEquals(attacker.getHp(), attacker.getMaxHp(context) - 2 * BASE_ATTACK - ATTACK_BONUS);
		Assert.assertEquals(defender.getHp(), defender.getMaxHp(context) - 2 * attacker.getAttack(context));
		Assert.assertEquals(defender.getAttack(context), BASE_ATTACK + 2 * ATTACK_BONUS);
	}
	...
	public void testSavageRoar() {
		...
		Assert.assertEquals(druid.getAttack(context), 1);
		...
		Assert.assertEquals(druid.getAttack(context), 3);
		Assert.assertEquals(minion.getAttack(context), 3);

		context.getLogic().endTurn(player.getId());
		Assert.assertEquals(druid.getAttack(context), 0);
		Assert.assertEquals(minion.getAttack(context), 1);
	...
	public void testSpitefulSmith() {
		...
		Assert.assertEquals(player.getHero().getWeapon().getWeaponDamage(context), 3);
		...
		Assert.assertEquals(player.getHero().getWeapon().getWeaponDamage(context), 5);
	...
	public void testBetrayal() {
		...
		Assert.assertEquals(targetMinion.getAttack(context), 3);
	...
	public void testEydisDarkbane() {
		...
		Assert.assertEquals(warrior.getHero().getHp(), warrior.getHero().getMaxHp(context) - 3);
	...
	public void testRallyingBlade() {
		...
		Assert.assertEquals(argentSquire.getAttack(context), 1);
		...
		Assert.assertEquals(argentSquire.getAttack(context), 2);
	...
	public void testCurseOfRafaam() {
		...
		Assert.assertEquals(player.getHero().getHp(), player.getHero().getMaxHp(context) - CURSE_OF_RAFAAM_DAMAGE);
