...
public class WeaponTests extends TestBase {
	...
	public void testWeapon() {
		...
		Assert.assertEquals(context.getLogic().getEntityAttack(warrior), 0);
		context.getLogic().receiveCard(player.getId(), weaponCard);
		context.getLogic().performGameAction(player.getId(), weaponCard.play());
		Assert.assertEquals(context.getLogic().getEntityAttack(warrior), 2);
	...
	public void testKingsDefenderAttendee() {
		...
		Assert.assertEquals(context.getLogic().getEntityAttack(hero.getWeapon()), 4);
		...
		Assert.assertEquals(context.getLogic().getEntityAttack(hero.getWeapon()), 3);
