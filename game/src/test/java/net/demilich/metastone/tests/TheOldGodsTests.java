...
public class TheOldGodsTests extends TestBase {
	...
	public void testDarkshireCoucilman() {
		...
		Assert.assertEquals(context.getLogic().getEntityAttack(darkshireCouncilman), darkshireCouncilman.getBaseAttack());
		...
		Assert.assertEquals(context.getLogic().getEntityAttack(darkshireCouncilman), darkshireCouncilman.getBaseAttack() + 1);
		Assert.assertEquals(context.getLogic().getEntityAttack(darkshireCouncilman2), darkshireCouncilman2.getBaseAttack());
		...
		Assert.assertEquals(context.getLogic().getEntityAttack(darkshireCouncilman), darkshireCouncilman.getBaseAttack() + 1);
		Assert.assertEquals(context.getLogic().getEntityAttack(darkshireCouncilman2), darkshireCouncilman2.getBaseAttack());
		Assert.assertEquals(context.getLogic().getEntityAttack(opponentMinion), opponentMinion.getBaseAttack());
	}
