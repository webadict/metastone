 ...
 * net.demilich.metastone.game.logic.GameLogic#fight(Player, Actor, Actor)} the {@code target}.
 * <p>
 ...
 * spell cast, the {@code target} as the defender, and the {@link EntityReference#SPELL_OUTPUT} set to the attacker.
 * <p>
 ...
 *     "spell": {
 *       "class": "FightSpell",
 *       "secondaryTarget": "SPELL_OUTPUT"
 *     }
 *   }
 * </pre>
 * Note that the {@link EntityReference#SPELL_OUTPUT} refers to the newly copied minion. Since the copied minion is in the
...
public class FightSpell extends Spell {
	...
	protected void onCast(GameContext context, Player player, SpellDesc desc, Entity source, Entity target) {
		EntityReference secondaryTarget = (EntityReference) desc.get(SpellArg.SECONDARY_TARGET, source == null ? EntityReference.NONE : source.getReference());
		List<Entity> resolvedSources = context.resolveTarget(player, source, secondaryTarget);
		if (resolvedSources == null) {
			logger.error("onCast {}: Could not resolve source key {} targeting {}", source, secondaryTarget, target);
			return;
		}
		...
		for (Entity resolvedSource : resolvedSources) {
			if (!(resolvedSource instanceof Actor)) {
				logger.error("onCast {}: Source entity {} targeting {} is not an Actor", source, resolvedSource, target);
				continue;
			}

			if (!(target instanceof Actor)) {
				logger.error("onCast {} {}: Target {} is not an Actor (trying to attack with {})", source, target, resolvedSource);
				continue;
			}
			...
			if (resolvedSource.equals(target)) {
				logger.warn("onCast {}: Source {} is trying to attack itself, which is not allowed. Skipping.", source, resolvedSource);
				continue;
			}
