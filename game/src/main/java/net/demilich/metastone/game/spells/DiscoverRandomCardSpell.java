...
public class DiscoverRandomCardSpell extends Spell {
	...
	protected void onCast(GameContext context, Player player, SpellDesc desc, Entity source, Entity target) {
		...
		HeroClassImplementation heroClass = (HeroClassImplementation) cardFilter.getArg(FilterArg.HERO_CLASS);
		if (heroClass == null) {
		...
		if (heroClass != NonHeroClass.NEUTRAL && !heroClass.isBaseClass()) {
			Card card = context.getPendingCard();
			heroClass = card.getHeroClass();
			if (heroClass == NonHeroClass.NEUTRAL) {
				heroClass = SpellUtils.getRandomBaseHeroClass();
			}
		}
		...
		if (heroClass == NonHeroClass.NEUTRAL) {
			...
			cards.addAll(CardCatalogue.query(context.getDeckFormat(), NonHeroClass.NEUTRAL));
